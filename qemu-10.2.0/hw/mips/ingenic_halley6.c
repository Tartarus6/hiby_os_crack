...
static void halley6_init(MachineState *machine)
{
    ...
    /* Set up CPU clock */
    s->cpuclk = qdev_get_clock_in(cpudev, "clk-in");
...
//                   115200, serial_hd(2), DEVICE_LITTLE_ENDIAN);
    ...
    if (machine->initrd_filename) {
        ...
        initrd_size = get_image_size(machine->initrd_filename,&error_fatal);
        if (initrd_size > 0) {
            ...
            if (load_image_targphys(machine->initrd_filename,
                                    ...
                                    initrd_size, &error_fatal) != initrd_size) {
...
static void halley6_machine_class_init(ObjectClass *oc, const void *data)
{
