...
## little bugs
- [x] fix progress slider jumping back briefly after seeking. this is likely due to the delay in performing the seek, so the old data is still being given from `audio_get_progress()` after the seek request occurred. could probably just set the current seconds in `audio.c` directly whenever the seek function is called?
