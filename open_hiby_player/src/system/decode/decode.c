...
decode_format_t decode_detect_format(const char *filepath) {
    if (has_extension(filepath, ".mp3")) return DECODE_FORMAT_MP3;
    if (has_extension(filepath, ".flac")) return DECODE_FORMAT_FLAC;
    if (has_extension(filepath, ".ogg")) return DECODE_FORMAT_OGG_VORBIS;
...
uint64_t decoder_read_pcm_frames_s16(decoder_t *dec, uint64_t frame_count, short *pBuffer) {
    ...
    case DECODE_FORMAT_OGG_VORBIS: {
        int frames_read = stb_vorbis_get_samples_short_interleaved(
            dec->impl.vorbis, dec->channels, pBuffer, (int)(frame_count * (uint64_t)dec->channels));
